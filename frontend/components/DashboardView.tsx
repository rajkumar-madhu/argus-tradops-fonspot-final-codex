...
export default function DashboardView({
  ...
  const sessionRows: any[] = (sessions?.items || [])
    .filter((s: any) => s.event === 'login')
    .slice(0, 5);
  ...
  > = {
    total: {
      value: fmt(total),
      delta: isJournal ? 'Unique orders in journal' : `${lookback} window`,
      tone: 'blue',
      icon: <ClipboardList size={18} />,
    },
    complete: {
      value: fmt(complete),
      delta: pct(complete, total),
      deltaTone: 'up',
      tone: 'green',
    ...
    active_users: {
      ...
      delta:
        activeUsers == null
          ...
          : `${fmt(sessionSummary.unique_users ?? activeUsers)} unique users`,
      tone: 'purple',
      icon: <UserRound size={18} />,
    },
    brokers: {
      value:
        brokersWithSessions == null
          ? '—'
          : `${fmt(brokersWithSessions)}${brokersInOrders ? ` / ${fmt(brokersInOrders)}` : ''}`,
      delta:
        brokersWithSessions == null ? 'Sessions not available' : 'With a session / seen in orders',
      tone: 'teal',
      icon: <Users size={18} />,
    },
  };
      ...
      <section className="ref-head">
        <div>
          <div className="ref-title-row">
            ...
            <span
              className={`source-badge ${isJournal ? 'file-based' : sourceBadgeTone(source, !ovErr)}`}
            >
              {sourceBadgeText(source, !ovErr)}
            </span>
          </div>
        ...
        <>
          <section className="kpi-grid ref-kpis six mission-kpis" aria-label="Dashboard KPIs">
            {MISSION_KPI_DEFS.map((def) => {
              ...
              return (
                <div key={def.key} title={def.definition}>
                  <KpiCard
                    label={def.label}
                    value={v.value}
                    delta={v.delta}
          ...
          <section className="ref-grid four mission-charts">
            <div className="panel">
              <div className="panel-head">
                ...
                <span className="legend">
                  <i className="lg s-total" /> Total <i className="lg s-executed" /> Executed{' '}
                  <i className="lg s-rejected" /> Rejected
                </span>
              </div>
              {orderTrend ? (
                <div className="ref-chart">
                  <AreaChart series={orderTrend.series} labels={orderTrend.labels} height={150} />
                </div>
              ) : (
                <EmptyState
                  title="No trend data"
                  body="Load orders to plot the operational timeline."
                />
              )}
            </div>
            <div className="panel">
              <div className="panel-head">
                <b>Order Distribution</b>
              </div>
              <div className="ref-donut">
                <Donut centerLabel="Total Orders" centerValue={fmt(total)} slices={donutSlices} />
              </div>
            </div>
            <div className="panel">
              <div className="panel-head">
              ...
              {apiError(rj) ? (
                ...
                <EmptyState
                  title="No rejections"
                  body="No rejected orders in the current window."
                />
              ) : (
                <div className="ref-chart">
                  <HBarList
            ...
            <div className="panel">
              ...
              {apiError(ex) ? (
                ...
                <EmptyState
                  title="No exchanges"
                  body="Exchange breakdown is empty for this source."
                />
              ) : (
                  ...
                  <tbody>
                    {exchangeItems.slice(0, 6).map((x: any) => (
                      <tr key={x.name}>
                        ...
                        <td>
                          <span
                            className={`ref-pill ${x.status === 'Connected' || x.status === 'Healthy' ? 'ok' : isJournal ? 'info' : 'warn'}`}
                          >
                            {isJournal ? 'Observed' : x.status || '—'}
                          </span>
                        </td>
                        <td className="num">
                          {x.reject_rate == null ? '—' : `${Number(x.reject_rate).toFixed(1)}%`}
                        </td>
                        <td className="mono">
                          {lastByExchange.get(x.name) ? istTime(lastByExchange.get(x.name)) : '—'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {apiError(od) ? (
            ...
            <MissionControlTable
              initial={od}
              source={source}
              windowQuery={windowQuery || `lookback=${lookback}`}
            />
          )}
          ...
          <section className="ref-grid four">
            <div className="panel">
              <div className="panel-head">
                <b>Network Bandwidth (WAN)</b>
              </div>
              <EmptyState
                title="No bandwidth source"
                body="Set PROMETHEUS_URL with node-exporter to chart interface throughput."
              />
            </div>
            <div className="panel">
              <div className="panel-head">
                <b>Recent Exchange Messages</b>
                <Link href="/logs?msg_type=yel_connected">View all ›</Link>
              </div>
              {yelRows.length ? (
                <table className="compact ref-table">
                  <thead>
                    ...
                    {yelRows.map((r: any) => {
                      ...
                      return (
                        <tr key={r.source_line}>
                          ...
                          <td>
                            <span className={`ref-pill ${state === 'CONNECTED' ? 'info' : 'warn'}`}>
                ...
                <EmptyState
                  title="No exchange messages"
                  body={
                    isJournal
                      ? 'The journal carries no yel_connected events in scope.'
                      : 'Exchange connect events are read from the journal explorer.'
                  }
                />
              )}
            </div>
            <div className="panel">
              ...
              {sessionRows.length ? (
                  ...
                  <tbody>
                    {sessionRows.map((s: any, i: number) => (
                      <tr key={`${s.user_id}-${i}`}>
                        ...
                        <td>
                          <span className={`ref-dot ${s.active ? 'ok' : 'idle'}`} />
                ...
                <EmptyState
                  title="No sessions"
                  body="No login events are available to this role or source."
                />
              )}
            </div>
            <div className="panel">
              ...
              {health ? (
                <ul className="ref-health">
                  {health.map((h) => (
                    <li key={h.name}>
                      <span>{h.name}</span>
                      <span className={`cc-pill cc-${h.tone}`}>{h.state}</span>
                    </li>
                  ))}
                </ul>
              ) : (
                <EmptyState
                  ...
                  body="Dependency status needs infrastructure access."
                />
              )}
            </div>
          </section>
          ...
          {fileStrip && (
            <section className="panel dashboard-source-strip">
              ...
              <p>
                Journal orders and CSV latency are separate observations; source dates may differ.
              </p>
