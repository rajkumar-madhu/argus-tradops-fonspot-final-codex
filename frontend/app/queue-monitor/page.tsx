...
export default async function Page({searchParams}:{searchParams:Promise<FileQuery>}){
 ...
 <div className="queue-trends">{(d.items||[]).filter((r:any)=>r.samples>0).map((r:any)=><section key={r.file} className="panel"><div className="panel-head"><b>{r.instance} backlog</b><span>Peak pending depth per {r.bucket_seconds} seconds</span></div><ObservedTrend unit="queue entries" label={`${r.instance} backlog`} points={(r.trend||[]).map((p:any)=>({time:p.time,value:p.peak}))}/></section>)}</div>
 </>}</Shell>;
}
