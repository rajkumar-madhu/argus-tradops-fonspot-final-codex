        ...
        with self.connection() as db, db:
            for row in db.execute('SELECT name FROM files').fetchall():
                if row['name'] not in {p.name for p in files}:
                    db.execute('DELETE FROM events WHERE file=?',(row['name'],));db.execute('DELETE FROM files WHERE name=?',(row['name'],))
        return self.catalog()

    def _ingest_file(self,path):
        start=time.perf_counter();kind='queue' if path.name.startswith('QueSize_') else 'latency'
        instance=path.name[len('QueSize_'):].rsplit('_',1)[0] if kind=='queue' else 'latency'
