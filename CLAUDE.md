...
- **Positions and Trades show no money.** Counts, quantities and per-unit prices only — no turnover, traded value, MTM or P&L (product decision, 2026-09-13). `lib/positions-flow.ts` derives fill counts/quantities; `tests/no-money-on-flow-pages.test.mjs` fails if `money()`/`inr()` or a value/MTM field reappears in `app/positions`, `app/trades` or `components/PositionsFlow.tsx`.
...
`app/tenancy.py` makes one deployment serve several clients. A **tenant** is a data source (an Elasticsearch cluster and/or a journal file) plus the users granted to it. With `TRADEOPS_MULTI_TENANT` unset there is one implicit tenant, `default`, built from the existing settings, so single-tenant deployments behave exactly as before. Enabling it refuses to start with `TRADEOPS_DEMO_MODE=true` (demo data would be served to every tenant).

- **Selection.** `bind_request_tenant` is an app-wide **async** dependency that reads `X-TradeOps-Tenant`, then `?tenant=` (EventSource), then the `tradeops_tenant` cookie, into a ContextVar. It must stay async: FastAPI runs sync dependencies in a copied context, so a value set there never reaches the endpoint (`tests/test_tenancy.py` fails if it is made sync). Only `/api/` paths are scoped; `/health` and `/metrics` always read `default`.
- **Access.** `auth.require()` calls `tenancy.assert_access(user)` after the permission check. `super_admin` (the `*` permission) sees every tenant; everyone else needs a row in `tenant_grants` matched on lower-cased Keycloak username, email or `sub` — including for `default` once multi-tenancy is on. Registry and grant lookups fail closed. Routes that take `current_user` directly must call `assert_access` themselves (`journal_routes.allowed` does). `require(..., tenant_scoped=False)` is only for tenant administration.
- **Isolation.** Anything cached must key on `tenancy.current_id()`: `cache.ttl_cache`, the ES field-resolution cache and `elastic.client.get_es()` (one client per tenant, URL and credential hash) do. Read journal paths via `tenancy.journal_path()`, never `settings.journal_path`; a non-default tenant's journal must resolve inside `TRADEOPS_TENANT_JOURNAL_DIR`.
- **Credentials never touch the database.** A tenant row names a `credentials_ref`; the key is read from `<TRADEOPS_TENANT_SECRETS_DIR>/<ref>.es_api_key` (or `.es_username` + `.es_password`), a mounted Kubernetes Secret. Keys must be read-only. The admin API never accepts or returns a credential.
- **Default-tenant-only features.** The collector, correlation worker and CSV store run for `default` only, so for any other tenant `/api/stream/{orders,rejections,exchange}` return 404, persisted incidents/RCA lists return `source: "unavailable"`, and `/api/files/*` returns 503. Market data is shared.
- **Admin API** (`app/tenant_routes.py`, `/admin/tenants` page, super_admin only): list, create, update/disable, test connection, and grants. Writes require a `Bearer` Authorization header and a JSON body (`header_token_required`) because the token is also accepted from a SameSite=None cookie — a test asserts every write route carries the guard. CORS therefore allows the write methods and the `X-TradeOps-Tenant` header.
- **Frontend.** `lib/tenant.ts` holds the cookie/header names (a test checks they match the backend). `serverApiHeaders()` in `lib/api.ts`, `authHeaders()` and `openAuthenticatedEventSource()` forward the tenant; a test fails if a server call or proxy stops doing so. `TenantSwitcher` adopts the tenant the API reports as current on first load (the API alone can only fall back to `default`) and reloads on change. `/admin/tenants` is listed in `lib/auth-routing.ts` so the middleware guards it.
- Migration `20260913_0003` adds `tenants` and `tenant_grants`; they stay empty until multi-tenancy is enabled.
...
- Exchange time comes from `ExchTimeStamp` alone. `ExchNsecs` is **not** a sub-second remainder like `NorenNsecs` — it is a full nanosecond timestamp on one of two epochs (unix on BSE/BFO/MCX, 1980-01-01 IST on NSE/NFO/CDS), so `normalizer.exchange_time()` takes its `% 1e9` fraction only when one of those readings reproduces `ExchTimeStamp`'s exact second, and `ExchTimeStamp == 315513000` (that 1980 epoch's zero) means unstamped and yields empty. Adding `ExchNsecs` dates every order in the 2070s–2080s. See `NOREN_FIELD_MAP.md`.
