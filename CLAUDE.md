...
- **Credentials never touch the database.** A tenant row names a `credentials_ref`; the key is read from `<TRADEOPS_TENANT_SECRETS_DIR>/<ref>.es_api_key` (or `.es_username` + `.es_password`), a mounted Kubernetes Secret. Keys must be read-only. The admin API never accepts or returns a credential. Production ConfigMap sets the dirs and the API Deployment mounts them (`k8s/tenant-secrets.example.yaml`); `TRADEOPS_MULTI_TENANT` stays false until `tenant_grants` includes the default tenant's operators. Startup refuses a missing secrets directory when the flag is on in `TRADEOPS_ENV=production`.
